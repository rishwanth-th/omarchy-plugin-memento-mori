...
`toggleProjection`, `toggleAnimation` and `moveInspection`. They are developer
tooling rather than part of this contract; see
...
If a change to this plugin's QML appears not to have taken effect, that is very
likely the reload hazard rather than the routing: see
[development.md](development.md).
